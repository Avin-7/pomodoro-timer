...
export class Service{
    ...
    async storeData({content,completed,email}) {
        try {
            await this.database.createDocument(conf.appWriteDatabaseId, conf.appWritetTodosCollectionId,ID.unique(), {
                content: content,
                completed: completed,
                email: email,
            });
    ...
    async getData({email}) {
        try {
            const res = await this.database.listDocuments(conf.appWriteDatabaseId, conf.appWritetTodosCollectionId,[
    ...
    async updateDocument(id, newContent) {
        try {
            ...
            return await this.database.updateDocument(conf.appWriteDatabaseId, conf.appWritetTodosCollectionId, id, {
                content: newContent,
            });
        } catch (error) {
    ...
    async deleteDocument(id) {
        try {
            await this.database.deleteDocument(conf.appWriteDatabaseId, conf.appWritetTodosCollectionId, id);
        } catch (error) {
